...
find_temp_highd <- function(list_ids){
  ...
  while(check_loop){
    if(alg=="PT_IIT_Z"){
      ...
      output <- PT_IIT_sim(p,1,total_simulations,total_iter,sample_inter_swap,burnin_iter,temperatures,bal_f,TRUE,"",0,0,3,num_modes)
    }
    if(alg=="PT_A_IIT"){
      ...
      output <- PT_a_IIT_sim(p,1,total_simulations,total_swap,sample_inter_swap,burnin_iter,temperatures,bal_f,"",0,0,0,"never",3,num_modes)
    }
