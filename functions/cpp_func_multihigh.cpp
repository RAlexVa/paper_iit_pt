...
double loglik(const arma::vec& X,const arma::sp_mat& M){
  ...
  int sizeX=X.n_elem;
  int mid_point=floor(sizeX/2);
  //Max registered likelihood is 9914
  //Adding a mode in the following states:
  // alternating 010101
  // first half of entires is 1 and the rest is 0
  //first half of entries is 0 and the rest is 1
  
  vec first_half=X.subvec(0,mid_point-1);
  vec second_half=X.subvec(mid_point,sizeX-1);
  
  double sum_fh=sum(first_half);
  double sum_sh=sum(second_half);
  
  if(sum_fh==mid_point & sum_sh==0){temporal+=10000;}
  if(sum_fh==0 & sum_sh==mid_point){temporal+=10000;}
